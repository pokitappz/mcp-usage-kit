...
- Authentication failures are counted separately from malformed-header
  rejections, so credential stuffing is distinguishable in the metrics. Rate
  limiting itself is the deployer's responsibility; stack a Tower rate-limit
  layer ahead of `MeterLayer`.
